...
public abstract class ShaderProgram {
	...
	private static final int DEFAULT_NUM_TEXTURES = 4;
	...
	protected final int[] location_textureUnits;
	private final HashMap<String, Integer> uniformLocations;
	private static final FloatBuffer matrix4 = BufferUtils.createFloatBuffer(16),
									 matrix3 = BufferUtils.createFloatBuffer( 9);
	
	/***/
	public ShaderProgram(final String computeFile) { this(computeFile, DEFAULT_NUM_TEXTURES); }
	
	/***/
	public ShaderProgram(final String computeFile, final int numTextures) {
		...
		for(int i = 0; i < attributeList.size(); i++) bindAttribute(i, attributeList.get(i));
		attributeList.clear();
		GL20.glLinkProgram(id);
		GL20.glValidateProgram(id);
		uniformLocations = new HashMap<>();
		if(MAX_TEXTURE_UNITS == -1) MAX_TEXTURE_UNITS = GL11.glGetInteger(GL20.GL_MAX_TEXTURE_IMAGE_UNITS);
		this.numTextures = (int)Maths.clamp(numTextures, 1, MAX_TEXTURE_UNITS);
		location_textureUnits = new int[this.numTextures];
	 ...
	 * and a fragment shader. By default, it creates {@value #DEFAULT_NUM_TEXTURES}
	 * texture slots.
	 * 
	 * @param vertexFile The path for the vertex shader file.
	 * @param fragmentFile The path for the fragment shader file.
	 * 
	 * @see #remapTextureSamplerName(int, String) */
	public ShaderProgram(final String vertexFile, final String fragmentFile) { this(vertexFile, fragmentFile, DEFAULT_NUM_TEXTURES); }
	 ...
	 * @param vertexFile The path for the vertex shader file.
	 * @param fragmentFile The path for the fragment shader file.
	 * @param numTextures The number of textures created in the buffer.
	 * More textures can still be created on the fly using 
	 * {@link #remapTextureSamplerName(int, String)} method.
	 * 
	 * @see #remapTextureSamplerName(int, String) */
	public ShaderProgram(final String vertexFile, final String fragmentFile, final int numTextures) {
		...
		uniformLocations = new HashMap<>();
		if(MAX_TEXTURE_UNITS == -1) MAX_TEXTURE_UNITS = GL11.glGetInteger(GL20.GL_MAX_TEXTURE_IMAGE_UNITS);
		this.numTextures = (int)Maths.clamp(numTextures, DEFAULT_NUM_TEXTURES, MAX_TEXTURE_UNITS);
		location_textureUnits = new int[this.numTextures];
		getAllTextureLocations();
	...
	public ShaderProgram(final String vertexFile, final String geometryFile, final String fragmentFile) { this(vertexFile, geometryFile, fragmentFile, DEFAULT_NUM_TEXTURES); }
	 ...
	 * 
	 * @see #remapTextureSamplerName(int, String) */
	public ShaderProgram(final String vertexFile, final String geometryFile, final String fragmentFile, final int numTextures) {
		final int vertexShaderID = loadShader(vertexFile, GL20.GL_VERTEX_SHADER),
	...
	// Hack to have two different behaviors for internal and external use
	 ...
	 * <br /><br />
	 * <b><u>NOTE</u></b> : <br>
	 * <i>1. The main reason for the existence of this method (other
	 * 		 than just aesthetic) is that it might be necessary for introducing
	 * 		 Materials to the shaders. In that case, structs will have to be
	 * 		 created and the new name of the texture samplers will be <code>
	 * 		 [struct_instance_name].[texture_name]</code>.<br>
	 * 	  2. This method <b>doesn't</b> need to be called every render
	 * 		 cycle. Calling it every frame may create some overhead. </i>
	 * 
	 * @param location The index location of the texture unit to have
	 * a new name.
	 * @param name The new name for the said texture unit.
	 * 
	...
	protected boolean remapTextureSamplerName(final int location, final String name) {
		if(location >= 0 && location < numTextures) {
			location_textureUnits[location] = getUniformLocation(name, false);
			GL20.glUniform1i(location_textureUnits[location], location);
			return true;
		}
		return false;
	}
	 ...
	 * texture bank so that it can be read from the correct texture input. It is assumed
	 ...
	 * without the parenthesis. Initially, only 5 texture units (position 0 to 4)
	 ...
	 * {@link #numTextures}. */
	private void getAllTextureLocations() { for(int i = 0; i < numTextures; i++) location_textureUnits[i] = getUniformLocation("texture" + i, false); }
