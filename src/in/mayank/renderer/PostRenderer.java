...
public class PostRenderer {
	...
	public PostRenderer(int width, int height, int colorType, int depthType, int samples, int targets) {
		fbo = new FBO(width, height, colorType, depthType, samples, targets);
	}
	...
	public int getOutputTexture(int attachment) {
		return (fbo != null) ? fbo.getColorTexture(attachment) : 0;
	}
	
	/***/
	public int getDepthTexture() {
		return (fbo != null) ? fbo.getDepthTexture() : 0;
	}
